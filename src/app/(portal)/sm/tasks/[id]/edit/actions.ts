...
const HUMAN_FIELD: Record<string, string> = { title: "Title", verticalId: "Vertical", subVerticalId: "Sub-vertical", priorityId: "Priority", deadline: "Deadline", frequency: "Frequency", source: "Source", expectedOutput: "Expected output", supportNeeded: "Support needed", delayReason: "Delay reason", nextAction: "Next action", intervention: "Dr. BN intervention", status: "Status" };
...
async function resolveLabels(patch: Record<string, unknown>, existing: any) { const labels: Record<string, { from?: string | null; to?: string | null }> = {}; if (!existing) return labels; if (patch.verticalId && patch.verticalId !== existing.verticalId) { const v = await prisma.vertical.findUnique({ where: { id: patch.verticalId as string }, select: { name: true } }); labels.verticalId = { from: existing.vertical?.name, to: v?.name }; } if ((patch.subVerticalId ?? null) !== (existing.subVerticalId ?? null)) { let to: string | null = null; if (patch.subVerticalId) { const sv = await prisma.subVertical.findUnique({ where: { id: patch.subVerticalId as string }, select: { name: true } }); to = sv?.name ?? null; } labels.subVerticalId = { from: existing.subVertical?.name ?? null, to }; } if (patch.priorityId && patch.priorityId !== existing.priorityId) { const p = await prisma.priority.findUnique({ where: { id: patch.priorityId as string }, select: { code: true, label: true } }); labels.priorityId = { from: existing.priority ? `${existing.priority.code} ${existing.priority.label}` : null, to: p ? `${p.code} ${p.label}` : null }; } return labels; }
